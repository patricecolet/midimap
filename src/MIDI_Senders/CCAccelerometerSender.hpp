 ...
 * 
 * The accelerometer readings are mapped to a 7-bit range (0-range) to be compatible with the MIDI standard. The hysteresis 
...
public:
     ...
     */
    CCAccelerometerSender(MIDIAddress addressX, MIDIAddress addressY, MIDIAddress addressZ, uint8_t range)
        : _addressX(addressX), _addressY(addressY), _addressZ(addressZ), _range(range) {}
     ...
     * 
     * This method reads the accelerometer values (X, Y, and Z axes), maps them to MIDI range (0-range), 
    ...
    void update() {
        ...
        // Map accelerometer values from raw range (-32 to 31) to MIDI range (0 to _range)
        uint8_t mappedX = map(x, -32, 31, 0, _range); 
        uint8_t mappedY = map(y, -32, 31, 0, _range); 
        uint8_t mappedZ = map(z, -32, 31, 0, _range); 
